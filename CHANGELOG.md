...
- Normal `/home` now prepares saved-home terrain as Minecraft's placement rules and OMWH's safety checks need it instead of loading a fixed area up front. Force still prepares the terrain Minecraft needs to resolve the home, but skips OMWH's safety checks.
- Normal `/spawn` now prepares terrain as each possible safe spot needs it while searching within 48 blocks of the spawn anchor on each axis. Cold terrain and longer searches can remain pending across server ticks instead of causing an immediate refusal.
